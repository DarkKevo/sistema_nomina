...
export default function AddHistorialBonificaciones({ update }) {
  ...
  const bonificaciones = useQuery('bonificaciones', () => fetch('http://localhost:3000/ListarBonificacion').then((res) => res.json()));
    ...
    <>
      ...
      <div className={`${openModal ? 'grid' : 'hidden'} fixed place-items-center top-0 left-0 w-full h-screen bg-black bg-opacity-80 z-10`}>
        <div className={`grid fixed bg-DarkBlue right-5 w-3/4 h-3/4 place-items-center z-10 rounded-lg`}>
          ...
          >
            <h2 className='text-white text-left w-full border-b-2 border-white p-3'>Ingrese los datos de la Bonificación</h2>
                ...
                {empleados.data.error
                  ...
                  : empleados.data &&
                    empleados.data.results.map((empleado) => (
                      <option key={empleado.idEmpleados} value={empleado.idEmpleados}>
                        {empleado.nombres}
                      </option>
                    ))}
              </select>
                ...
                {bonificaciones.data.error
                  ...
                  : bonificaciones.data &&
                    bonificaciones.data.map((bon) => (
                      <option key={bon.idbonificaciones} value={bon.descripcion_bonificacion}>
