...
export default function HistorialDeducciones() {
  ...
  const mutation = useMutation(
    (datos) => {
      const res = fetch("http://localhost:3000/EliminarHistoriald", {
        method: "DELETE",
            ...
            {deduccionesHisto.data && (
              <tbody>
                {deduccionesHisto.data.map((deduccion) => (
                  <tr key={deduccion.id_d}>
                    <td className={tableStyle}>{deduccion.nombres}</td>
